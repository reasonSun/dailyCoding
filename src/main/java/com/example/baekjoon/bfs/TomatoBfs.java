package com.example.baekjoon.bfs;

import java.util.LinkedList;
import java.util.Queue;
import java.util.Scanner;

class TomatoPoint{
    int x, y;

    TomatoPoint(int x, int y) {
        this.x = x;
        this.y = y;
    }
}
public class TomatoBfs {
    static int[][] dis, board;
    static int[] dx = {-1, 0, 1, 0};
    static int[] dy = {0, 1, 0, -1};
    static int n, m;
    static Queue<TomatoPoint> queue = new LinkedList<>();
    public void bfs(){

        while (!queue.isEmpty()) {
            TomatoPoint tmp = queue.poll();
            for (int i = 0; i < 4; i++) {
                int nx = tmp.x + dx[i];
                int ny = tmp.y + dy[i];
                if (nx >= 0 && nx < n && ny >= 0 && ny < m && board[nx][ny] == 0) {
                    board[nx][ny] = 1;
                    queue.offer(new TomatoPoint(nx, ny));
                    dis[nx][ny] = dis[tmp.x][tmp.y] + 1;
                }
            }
        }
    }
    public static void main(String[] args) {
        TomatoBfs tomatoBfs = new TomatoBfs();
        Scanner scanner = new Scanner(System.in);

        m = scanner.nextInt();
        n = scanner.nextInt();
        dis = new int[n][m];
        board = new int[n][m];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                board[i][j] = scanner.nextInt();
                if(board[i][j] == 1) queue.offer(new TomatoPoint(i, j));
            }
        }

        tomatoBfs.bfs();

        boolean flag=true;
        int answer=Integer.MIN_VALUE;
        // board에 0이 하나라도 있으면 완벽하게 토마토가 익은게 아니기 때문에 false
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++){
                if(board[i][j]==0) flag=false;
            }
        }
        if(flag){
            for(int i=0; i<n; i++){
                for(int j=0; j<m; j++){
                    answer=Math.max(answer, dis[i][j]);
                }
            }
            System.out.println(answer);
        }
        else System.out.println(-1);
    }
}
